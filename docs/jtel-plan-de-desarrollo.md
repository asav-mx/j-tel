...
|---|---|---|
| **Tramo 3 — arreglar el árbitro.** Las 26 causas medidas del motor, contadas por lo que su propia tabla dice de cada una: **19 sin construir**, **4 partidas o a medias** (C1, C17, C20, C24), **2 cerradas** (C12, C15) y **1 que dejó de ser causa** (C10) | `docs/archivo/PLAN.md` §4 «Tramo 3» y §5.1 con la medición de cada una | Abierto. La regla que lo gobierna sigue en pie: **una causa por PR, y nunca dos términos de `servedRoute` en el mismo**. **C24 queda cerrada el 29** (23-sep): su mitad barata ya estaba en `main` desde el 12-ago (#291) —el expediente lee la política del sello— y la otra mitad es el acta dentro del hecho (**#540**, migración 0056), con el contorno de la evidencia para poder decir «esto ya no cuadra». Lo que NO cierra y sigue siendo causa aparte: la geocerca divergente, que es C4 |

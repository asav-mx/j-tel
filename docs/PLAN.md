...
| **C14** | **C19** | ⚠ **11 ago — no es dependencia de causa sino de FRONTERA, y es de calendario:** las dos deciden lo mismo —`pendiente_evidencia` contra veredicto— así que **moverlas en el mismo tramo hace inatribuible el efecto de las dos**, igual que dos términos del nudo. 🟢 **C14 vacía la pila** (los 64 `llegada_sin_atribucion` salen de pendiente) **y el piso de densidad la llena**, así que el orden decide contra qué se mide cada una. **Cuál va primero lo decide Asav** — el argumento de las dos está en `Ficha-Diseno-C14-Puerta-Sin-Salida.md` §5 |

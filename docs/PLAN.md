...
| **El contador anónimo** | ⚠ **La primera ruta de ESCRITURA de la cara pública, y la regla de arriba se corrige aquí en vez de dejarla mintiendo.** `POST /api/circuitos/[slug]/apertura`, sin autenticación, escribe **una fila por aparato distinguible por día**: huella `HMAC(llave; ip + agente + día + circuito)` derivada **en el servidor**, que rota cada día. **Nada se guarda en el teléfono** —ni cookie ni `localStorage`—, así que la deduplicación vive en el índice único de la base. Mide **aperturas, no regresos**: ligar un aparato entre días es justo lo que la rotación impide, y medir regresos sería otra decisión con su propio consentimiento. Es evento propio y explícito porque contar desde el tráfico existente **no funciona**: la forma se cachea 300 s en el CDN y las unidades se piden cada 15 s. Lo que compensa la escritura abierta: la misma puerta que el resto —un circuito sin publicar contesta como un slug inventado—, el HMAC encareciendo inflar, y la regla del firewall que ya apunta a `/api/circuitos/*`. **Se enseña un número y se guardan dos**: el crudo no sale a la pantalla porque es el detector de raspado, no una cifra de uso. Ver `Ficha-Contador-Anonimo.md` |
...
| **C11** | **Servicios con evidencia y sin atribución** | **Investigación CERRADA** — `Reporte-Final-Investigacion-71.md`; el detalle, en `Ficha-Diagnostico-Pendientes-Sin-Atribucion.md`. La causa raíz es **C19**. 🟢 **6 ago, por causa × sitio × turno:** **106** pendientes (eran 100 el 4 ago y 104 el 5) — `llegada_sin_atribucion` **61**, *todos* de **Planta 47 · Turno A**; cobertura insuficiente **28**; observación insuficiente **12**; sin evidencia **5**. Lo que la produce: **ninguna candidata cumple A (cobertura ≥ 60 %) y B (corredor ≥ 60 %) a la vez**. 🟢 **12 ago — remedido con instrumento propio (`medir-pendientes`), y cada cubo mapea contra el del 6 de agosto: 139 pendientes** — `llegada_sin_atribucion` **87** (61) · cobertura insuficiente **28** (**28, sin moverse**) · observación insuficiente **19** (12) · sin evidencia posible **5** (5). ⚠ **Lo que crece es uno solo: `llegada_sin_atribucion`, +26 en seis días**, y es C14. **Los otros tres cubos están quietos o casi** — la cobertura insuficiente ni se movió, que es lo que decía C3: es cicatriz, no hemorragia | **Medida.** Falta decidir el arreglo |
| **C14** | **`routeStrictness` no gobierna lo que su nombre promete** | Se lee en **un solo punto** (`packages/verification/src/index.ts:1018`) y solo elige entre `pendiente_evidencia` y `no_cumplido` **después** de que la atribución falló. La comprobación A∧B vive en `servedRoute` (`index.ts:575`) y **lo único que la apaga es `!hasKml`**. 🟢 **6 ago, y esto cierra el argumento: los 48 ruta-turnos de los dos contratos reales tienen KML activo con waypoints.** O sea `!hasKml` **no se dispara nunca hoy**, así que A∧B corre siempre y `destino_only` está **inerte para todos los servicios reales**. Con `destino_only`, una unidad que llegó **no puede salir `cumplido` jamás**. ⚠ **11 ago — esa frase es imprecisa y se corrige, porque leída literal la desmienten 92 cumplidos.** La exacta es más estrecha y más fuerte: una unidad que llegó **y no acredita el trazado** no puede salir `cumplido` jamás, **y como no hay otra vía, `destino_only` nunca ha acreditado a nadie por llegar**. 🟢 **La prueba es la distribución, no el argumento: de los 92 cumplidos de Planta 47, los 92 traen cobertura ≥ 60 % — mínimo 60.67 %, mediana 90.1 %.** Está truncada exactamente en el umbral; si la llegada acreditara sola habría cumplidos por debajo, y no hay ni uno. **La cláusula no está floja: no existe.** 🟢 **Población al 11 de agosto: 92 cumplido · 309 no cumplido · 103 pendiente, de los cuales 87 `llegada_sin_atribucion`** (eran 61 el 6 ago). ⚠ **12 ago — el 64 que decía esta fila era mío y estaba mal clasificado**: se tomaba el último asiento del ledger y hay asientos informativos que se escriben después del que decidió. Con el último asiento QUE JUZGA son 87. Ver `Ficha-Diseno-C14-Puerta-Sin-Salida.md` | **Sin construir, con ficha de diseño.** Es la causa de que los 87 no se puedan cerrar solos. ⚠ **Comparte frontera con el piso de densidad de C19** —las dos deciden `pendiente_evidencia` contra veredicto—, y **cuál la toca primero lo decide Asav** con el argumento de las dos en §5 de la ficha |
...
| **C16** | **La configuración del contrato no coincide con lo acordado, y no hay contra qué comparar** | 🟢 **6 ago: difieren dieciséis de 24 campos** entre los dos contratos — **no seis**, como decía este plan. Los que deciden un veredicto: `routeStrictness` · `kmlCorridorMinPct` (**60 vs 50**) · `kmlCorridorMeters` (120 vs 150) · `evidenceMarginMinutesAfter` · `verificationGraceMinutes` · `excusableReasons` (Planta 47 admite `falla_mecanica`, el Campus no) · `enforcementRules` (el Campus está **vacío**). 🟢 **Y siete existen solo en Planta 47**, así que el Campus corre con el valor de fábrica sin que nadie lo decidiera: `maxWindowBeforeMinutes` · `routeAvgSpeedKmh` · `routeDurationMinSamples` · `routeDurationPercentile` · `windowDerivationEnabled` · `windowSlackPct` · y **`timeZone`**, que es el campo del que salió D1. 🔵 Asav recordaba el corredor del Campus **acordado en 60 %**. 🟢 **Ya no difiere `kmlOriginToleranceFraction`**: se escribió en el Campus el 6 de agosto con `0.15`, el mismo de fábrica — **cambió quién manda, no el comportamiento**. Con `contract_policy_history` vacía (C13), **no hay forma de leer del sistema qué se pactó**. 🟢 **11 ago — remedido campo por campo, con guion reproducible (`comparar-politicas`), y la cifra se mueve por dos razones fechadas, no porque se haya arreglado nada: 8 difieren · 7 solo en Planta 47 · 10 coinciden, sobre 25 campos.** El esquema pasó de 24 a 25 cuando C12 metió `frechetMaxKm` (#269, 8 ago), y `kmlOriginToleranceFraction` dejó de diferir el 6 de agosto. ⚖ **Y una corrección que importa para no volver a confundirlos: `kmlMatchMinPct` —el umbral de la COBERTURA— está en 60 en los dos.** Lo que difiere es `kmlCorridorMinPct` (60 vs 50), que es el de precisión de corredor. 🟢 **Los siete de fábrica del Campus no son siete ajustes sueltos: seis son la misma pieza —el dimensionado de la ventana de observación—, así que nadie configuró con qué evidencia se juzga al Campus.** El séptimo es `timeZone`, de donde salió D1. Ver `Ficha-Insumo-C16-Que-Se-Pacto.md` | **Sin construir.** Más grave que un umbral flojo: el árbitro puede estar aplicando una regla que las partes no pactaron. 🟢 **11 ago — y ahora tiene precio medido:** con `kmlMatchMinPct` igual en los dos, **el ancho del corredor solo (150 m vs 120 m) decide que a 120 s de muestreo Planta 47 se quede en cero acreditadas y el Campus conserve 23**. La configuración no solo decide veredictos: **decide cuánta ceguera aguanta el sitio antes de volverse injuzgable** |
...
| **C26** | **Dos defensas que no coinciden en qué cuenta como cambio dejan un camino sin rastro** | 🟢 **13 ago, encontrada al leer el registro de política.** Hay dos guardas sobre la misma escritura y **cada una define «cambio» distinto**. El trigger de la 0020 dispara con **diferencia CRUDA** —`NEW.policy IS DISTINCT FROM OLD.policy`—. `updatePolicy` **apaga el trigger ANTES del UPDATE** (`set_config(jtel.registrado,1)`) y después escribe su propia fila **solo si `huboCambio`**, comparando la política **canónica efectiva** — con buena razón: sin canonizar, el primer guardado de un contrato viejo registraba seis cambios que nadie hizo. 🟢 **El problema no es ninguna de las dos por separado: es que cuando corre la aplicación, su definición gana.** Una edición **crudamente distinta y canónicamente igual** no deja rastro **por ningún lado** — el trigger estaba apagado y la aplicación decidió no escribir. ⚠ **Y lo que la hace difícil de ver: las dos defensas se ven bien aisladas.** El trigger dispara —comprobado contra la rama desechable: escribe su fila y firma `sql_directo`— y la aplicación registra lo que considera cambio. **Nadie que audite una de las dos encuentra nada.** 🟢 **Hoy no ha mordido:** `contract_policy_history` está en cero porque desde que el trigger existe (7 ago) nadie ha editado — las dos ediciones reales, Planta 47 el 31 jul y el Campus el 6 ago, son anteriores. **El camino existe y está sin recorrer** | **Sin construir.** **La forma del arreglo, dicha antes de elegirla:** o las dos guardas comparten la definición de «cambio», o **la aplicación deja de apagar el trigger** y se acepta la fila duplicada —que es fea y no pierde nada—. ⚠ **Lo que NO se puede hacer es quitar una de las dos**: cada una cubre un camino que la otra no —la consola y los guiones crudos, contra la firma de quién editó—. Es familia de la regla 8 y de la 24: **una defensa que se apaga sola a petición de otra no es una defensa, es una convención entre dos** |
...
| **C16** | **C19** | 🟢 **11 ago — nuevo, y salió de medir:** la configuración del contrato fija **cuánta densidad necesita un sitio para ser juzgable**. ⚠ **Corregido el mismo día al enumerar la política para C16:** el umbral de la cobertura es `kmlMatchMinPct` y **está en 60 en los dos contratos**, así que lo único que separa a los sitios es **el ancho del corredor, 150 m contra 120 m**. Con el mismo adelgazamiento a 120 s, **Planta 47 se queda en cero acreditadas y el Campus conserva 23**; a 60 s pierden **38 % contra 26 %**. **Treinta metros de corredor compran esa diferencia**, y nadie eligió ese intercambio porque nadie lo tenía medido |
...
| **—** | **C12 · C15 · C9 · C20 · C23** | 🟢 No dependen de nada ni desbloquean nada: **no mueven un veredicto**, mueven lo que el expediente dice y lo que la ley exige. 🟢 **C23 entra aquí con una diferencia:** las otras cuatro dicen algo falso sobre lo que pasó; **C23 promete algo que no va a pasar** |
| **C14** | **C19** | ⚠ **11 ago — no es dependencia de causa sino de FRONTERA:** las dos deciden lo mismo, `pendiente_evidencia` contra veredicto. 🟢 **12 ago — y medirlo aflojó la tensión: comparten frontera y NO comparten población.** Con el piso expresado como hueco mediano del mejor candidato, a 55 s tocaría **50 `no_cumplido` de Planta 47 y CERO pendientes**; C14 mueve **87 `llegada_sin_atribucion`**, que ya son pendientes. **Los dos conjuntos son disjuntos hoy**, así que **se pueden hacer en cualquier orden y cada una sigue siendo medible**, siempre que cada una reporte su propia población. **Compartir frontera no es compartir población** — y por eso la regla del nudo no aplica aquí igual. **El orden lo decide Asav**; el argumento de las dos está en la ficha de C14 §5 |
...
| 🆕 **Qué se pactó de verdad en el Campus** | **Conversación con la Planta** | 🟢 **11 ago — con insumo listo y reproducible:** `Ficha-Insumo-C16-Que-Se-Pacto.md` trae los **8 campos que difieren, los 7 que solo existen en Planta 47 y los 10 que coinciden**, cada uno con qué decide, y **seis preguntas concretas** ordenadas por lo que cuestan. 🔵 El corredor del Campus está en **50 %** y Asav lo recordaba en **60 %**. 🟢 **Y ahora la conversación tiene precio, no solo contenido:** treinta metros de corredor deciden que a 120 s de muestreo un sitio se quede en cero acreditadas y el otro conserve 23. **Sin esto, C16 no se puede cerrar: no hay contra qué comparar** | C16 · C13 · **C19** |
| 🆕 **Cuál toca primero la frontera pendiente/veredicto: C14 o el piso de densidad de C19** | Negocio + motor | 🟢 **11 ago — planteada con el argumento de las dos, sin decidir.** **A favor de C14:** es un **defecto** y el piso es **política nueva**; **C14 vacía la pila y el piso la llena**, así que hacerlo al revés entierra el efecto de C14 bajo pendientes nuevos; y **C14 no necesita D3 y el piso sí**. **A favor del piso:** ataca daño que **se emite hoy** —un `no_cumplido` sellado con densidad que no alcanzaba a ver rompe la Ley 7 ahora—, mientras que 🔵 **los 64 de C14 no acusan a nadie** porque están en pendiente. **La tensión se resuelve hacia arriba:** con D3 el piso deja de ser peligroso; con la respuesta de la Planta, C14 deja de depender de C16. **Y hay una tercera salida: el piso entra primero APAGADO** —construido, medido en seco, sin efecto— y se enciende después de C14. Ver `Ficha-Diseno-C14-Puerta-Sin-Salida.md` §5 | C14 · C19 · D3 |
| 🆕 **Si `destino_only` es de verdad lo que Planta 47 contrató** | **Conversación con la Planta** | 🟢 **11 ago:** es la pregunta 1 de la ficha de C16 vista desde el otro lado. **Si la respuesta es «no», C14 deja de ser un defecto del motor y pasa a ser una configuración mal escrita**, y el arreglo es de datos, no de código. 🟢 Y el precio de equivocarse está medido: pasar Planta 47 a `kml_full` convertiría los **64 `llegada_sin_atribucion`** en **64 acusaciones nuevas** | C14 · C16 |
| 🆕 **Si el arreglo de C19 cambia la métrica o solo pone un piso de densidad** | Negocio + motor | 🟢 **11 ago — anotada con su dependencia escrita, a pedido de Asav: se decide con la ficha de C16 enfrente, no antes.** El piso depende del corredor y del umbral, y **el corredor del Campus es justo lo que C16 no puede acreditar**. Cambiar la métrica cambia cómo se sella un hecho; poner un piso cambia qué se sella como pendiente — **solo lo segundo es reversible sin re-verificar**. Ver `Ficha-Diseno-C19-Cobertura-Y-Densidad.md` §4 | C19 |
| 🆕 **Qué pasa con el piso de densidad y D3** | **Negocio + legal** | 🟢 **11 ago — anotada con su dependencia escrita.** Un piso de densidad **manda servicios a `pendiente_evidencia`**, y 🔵 los pendientes ya son **106, con 89 de más de 48 h**. **Sin regla de cierre se cambia una acusación falsa por una pila que nadie cierra.** Va con la anterior y con D3, no suelta | C19 · D3 |
...
| **La reconciliación del expediente** | **C15 · C17 · 🆕 D3** | 🟢 El expediente hoy etiqueta mal la evidencia y, en todo lo sellado antes del 5 de agosto, solo tiene la cobertura ponderada. ✅ **12 ago — Asav le da su papel principal: es LA SALIDA REAL de los 139 pendientes.** Que el **carrier compile lo que sí hizo y lo presente**, y **la planta evalúe**. 🟢 Y encaja con lo medido: **ningún pendiente vuelve a la cola por su cuenta**, así que sin un camino humano **no hay ninguno**. **No es un cuarto veredicto** —los tres siguen siendo tres—: es que un pendiente **deje de ser un callejón** |
...
**11 de agosto de 2026, cierre (el insumo de C16 — y una corrección al PR
anterior, que salió de hacerlo).**

- 🟢 **C16 tiene insumo, medido campo por campo y reproducible**
  (`pnpm --filter @jtel/db comparar-politicas`, que es la mitad de medición del
  **quinto sensor**, el de divergencias de política). Ficha:
  `Ficha-Insumo-C16-Que-Se-Pacto.md`, con **seis preguntas concretas para la
  Planta** ordenadas por lo que cuestan si la respuesta es «eso no fue lo que
  acordamos».
- **La distinción que la ficha existe para hacer visible**, porque en un JSON las
  tres se ven igual: **difiere** (los dos lo declaran distinto) · **solo en uno**
  (uno lo declara y el otro **corre con el valor de fábrica sin que nadie lo
  eligiera**) · **igual**. **Un campo ausente no es un campo en blanco: es el
  `default` aplicándose en silencio.**
- 🟢 **8 difieren · 7 solo en Planta 47 · 10 coinciden, sobre 25 campos.** ⚠ La
  cifra se movió respecto del «dieciséis de 24» del 6 de agosto **por dos razones
  fechadas y ninguna es que se haya arreglado algo**: el esquema creció a 25 con
  `frechetMaxKm` (C12, #269) y `kmlOriginToleranceFraction` dejó de diferir el 6
  de agosto. **La lista se volvió a medir, no a corregir.**
- 🟢 **Los siete de fábrica del Campus no son siete ajustes sueltos: seis son la
  misma pieza** —`windowDerivationEnabled`, `windowSlackPct`, `routeAvgSpeedKmh`,
  `routeDurationPercentile`, `routeDurationMinSamples`, `maxWindowBeforeMinutes`—,
  o sea **el dimensionado de la ventana de observación**. **Nadie configuró con
  qué evidencia se juzga al Campus.** El séptimo es `timeZone`, de donde salió D1.
- 🟢 **Y una que no estaba en ninguna lista: `shiftCloseMinutesAfterStart` no lo
  declara ninguno y no tiene valor de fábrica.** Ningún contrato real tiene hora
  de cierre de turno.
- ⚠ **CORRECCIÓN al #278, y el error era mío.** La medición del efecto de
  densidad comparó la cobertura contra **`kmlCorridorMinPct`** —el umbral de
  **B**— en vez de contra **`kmlMatchMinPct`**, que es el de **A**. 🟢 **Planta 47
  tiene los dos en 60, así que su tabla no cambió ni un dígito.** El Campus los
  tiene en **60 y 50**, así que su tabla estaba medida contra 50. Corregida:
  **155 acreditan (no 235) con 79.5 % de cobertura mediana (no 69.6 %)**; a 60 s
  quedan **115** y a 120 s **23 (no 97)**.
- **Lo que hay que registrar del error, que es lo que sirve:** *un umbral
  equivocado que coincide en un contrato no se ve mal en ningún lado.* La corrida
  termina, imprime cifras plausibles y **solo el otro contrato lo delata**. No lo
  atrapó revisar el guion ni ninguna prueba: **lo atrapó enumerar los campos de la
  política para otra causa.** Dos umbrales que se parecen en el nombre y deciden
  cosas distintas son una trampa de nombre — **misma familia que C20**.
- 🟢 **Y el cruce C16 → C19 sobrevive, más chico y más limpio:** con
  `kmlMatchMinPct` igual en los dos, lo único que separa a los sitios es **el
  ancho del corredor, 150 m contra 120 m**. A 120 s de muestreo **Planta 47 se
  queda en cero y el Campus conserva 23**; a 60 s pierden **38 % contra 26 %**.
  **Treinta metros compran esa diferencia.** El hallazgo se achicó al corregirlo y
  pasó de «dos configuraciones dan fragilidades distintas» a **un campo, aislado,
  con su efecto medido**.
- ✅ **Las dos decisiones de C19 quedan anotadas en §5.5 como pendientes de Asav,
  con su dependencia escrita:** se toman **con la ficha de C16 enfrente**, porque
  el piso depende del corredor y el corredor del Campus es justo lo que no se
  puede acreditar.
- 🟢 **`servedRoute` sigue intacta.** Nada de esto toca el motor.

**11 de agosto de 2026, cierre del día (C14 con ficha, C23 nueva, y una frase de
§5.1 corregida por 92 cumplidos).**

- ⚠ **Se corrige el enunciado de C14, y lo corrige un dato.** §5.1 decía «con
  `destino_only`, una unidad que llegó **no puede salir cumplida jamás**», y leído
  literal **lo desmienten los 92 cumplidos de Planta 47**. La frase exacta es más
  estrecha y más fuerte: una unidad que llegó **y no acredita el trazado** no
  puede salir `cumplido` jamás, **y como no hay otra vía, `destino_only` nunca ha
  acreditado a nadie por llegar**.
- 🟢 **Y la prueba es la distribución, no el argumento: de los 92 cumplidos, los 92
  traen cobertura ≥ 60 % — mínimo 60.67 %, mediana 90.1 %.** Está **truncada
  exactamente en el umbral**. Si la llegada acreditara sola habría cumplidos por
  debajo de 60, y no hay ni uno. **La cláusula del contrato no está floja: no
  existe.** 🟢 **Población al 11 de agosto: 92 cumplido · 309 no cumplido · 103
  pendiente, de los cuales 64 `llegada_sin_atribucion`** — eran 61 el 6 de agosto.
- **Las opciones de C14, con veredicto** (`Ficha-Diseno-C14-Puerta-Sin-Salida.md`):
  ❌ **apagar A∧B con `destino_only`** —las 21 rutas de Planta 47 llegan a la misma
  geocerca, así que toda unidad que llegue acreditaría todas: no afloja la
  atribución, **la destruye**—. ✅ **A∧B deja de ser requisito y pasa a ser
  desempate**, que es la forma que encaja y **usa el pase de exclusividad que ya
  existe**; necesita un piso. ✅ **identificación por capas**, que es hacia donde
  apunta el plan y de la que la anterior es el primer peldaño. ❌ **pasar Planta 47
  a `kml_full`** —la barata, y por eso la tentadora—: **es decidir por el cliente
  qué contrató**, y 🟢 **convertiría los 64 pendientes en 64 acusaciones nuevas**.
- ⚠ **La frontera compartida queda planteada, no decidida, y entra a §5.5 y a
  §5.2 como arista `C14 → C19`.** C14 y el piso de densidad deciden **la misma
  línea** —pendiente contra veredicto—, así que moverlas en el mismo tramo hace
  inatribuible el efecto de las dos. **A favor de C14:** es defecto contra política
  nueva, **vacía la pila mientras el piso la llena**, y no necesita D3. **A favor
  del piso:** ataca daño que se emite hoy, mientras que los 64 de C14 no acusan a
  nadie. **Y una tercera salida: el piso entra primero apagado y se enciende
  después de C14** — las separa en el tiempo sin separarlas en el calendario.
- 🆕 **C23, propuesta: una perilla del contrato que se escribe, se explica y no la
  lee nadie.** 🟢 `shiftCloseMinutesAfterStart` aparece **solo** en el esquema, en
  las dos ramas de escritura del formulario y en el catálogo de perillas. **Ningún
  archivo la lee.** 🟢 Y la escritura está cuidada —hay un comentario explicando
  que vaciarla la BORRA en vez de caer al valor anterior—: **alguien pensó el ciclo
  de vida de un dato que nadie consume.** ⚠ **El comentario del esquema miente
  sobre el código**: dice que «la pantalla trata la ausencia como *turno histórico
  sin hora de cierre*», y esa pantalla no existe. **Es la familia de C9 al revés** —
  allá faltaba el último eslabón de una cadena sin principio; aquí están el
  esquema, la escritura, la pantalla y el texto que la explica, **y falta el que la
  lea**. 🟢 **Hoy no ha engañado a nadie porque nadie la ha puesto**, pero es
  editable y su texto promete un comportamiento concreto: **quien la configure va a
  creer que cambió algo.**
- **Y la respuesta a «qué implica no tener hora de cierre de turno», que es lo que
  se preguntó:** 🟢 **hoy no implica nada, y ése es el hallazgo.** No es que los
...
- 🟢 **D3 tiene instrumento propio para que su cifra deje de caducar:**
  `pnpm --filter @jtel/db medir-pendientes`, que es la mitad de medición del
  **sexto sensor** —«observación contra conducta»—. Es el tercero que se
  construye; faltan tres.
- 🟢 **139 pendientes · 118 de más de 48 h (84.9 %) · 100 de más de 7 días.** Eran
  106 y 89 el 6 de agosto, y 71 al abrirse D3. **Y cada cubo mapea contra el corte
  viejo**, que es lo que permite leer el movimiento: `llegada_sin_atribucion` **61
  → 87** · cobertura insuficiente **28 → 28** · observación insuficiente **12 →
  19** · sin evidencia **5 → 5**.
- 🟢 **Lo que crece es uno solo, y es C14:** `llegada_sin_atribucion`, **+26 en
  seis días**. Los otros tres cubos están quietos. **La cobertura insuficiente no
  se movió ni una unidad**, que es exactamente lo que decía C3: cicatriz de una
  tanda, no hemorragia. ⚠ **Y la pila no solo crece: envejece.** El más viejo lleva
  **34 días**.
- ⚠ **CORRECCIÓN al #280: el «64 `llegada_sin_atribucion`» era mío y estaba mal
  clasificado.** Se tomaba **el último asiento del ledger** de cada ocurrencia, y
  hay asientos informativos —`llegada_fuera_ventana`, el retiro de la cola— que se
  escriben **después** del que decidió y no traen paso de decisión. Esas
  ocurrencias caían a un cajón de sobrantes **que parecía plausible**: el total
  cuadraba y **solo el reparto por causa mentía**. Con el último asiento **que
  juzga**, son **87**. Es la §D del Marco —la agrupación— aplicada a una consulta.
- **La valla que deja, porque la cifra sola no sirve:** el guion ahora **separa
  «nunca se juzgó» de «tiene ledger y ningún paso que juzgue»**. Juntarlos fue el
  error, y juntos **convierten un defecto de la consulta en dato de la operación**.
  🟢 De paso quedó nombrado el cuarto cubo: los 5 del Campus son
  `sin_evidencia_posible` —**cero puntos**, retirados de la cola—, o sea el único
  pendiente que **no es del árbitro sino del aparato**, y el único que D3 puede
  cerrar sin re-verificar nada: esperar no lo va a resolver.
- 🟢 **El piso de densidad, dimensionado — y es chico.** Con el piso expresado como
  *hueco mediano del mejor candidato del servicio*, a **55 s** tocaría **50 de los
  309 `no_cumplido` de Planta 47**, **6 de los 343 `cumplido` del Campus** y
  **CERO pendientes**. **A 90 s no toca a nadie.** 🟢 **Entre 45 y 55 s la cifra no
  se mueve: hay meseta**, así que el umbral no es un filo.
- 🟢 **Y eso afloja la tensión de la frontera, que era la pregunta:** C14 mueve
  servicios **desde** pendiente y el piso los mueve **hacia** pendiente **desde
  `no_cumplido`** — **medido hoy, los dos conjuntos son disjuntos**. **Se pueden
  hacer en cualquier orden y cada una sigue siendo medible**, siempre que cada una
  reporte su propia población. **Compartir frontera no es compartir población.**
- ⚠ **El costo que no es bueno, dicho aparte:** los **6 cumplidos del Campus** que
  cruzan el piso **perderían una acreditación ya dada**. Quitarle un cumplido a un
  carrier que ya lo tenía **es un daño distinto** de no acusarlo.
- 🟢 **Qué costaría «el piso apagado»: poco, y por una razón que no se había
  dicho — casi todo ya existe.** La **frontera** ya tiene compuerta
  (`cobertura_evidencia`, con `evidenceMinCoveragePct` 80 y `evidenceMaxGapMinutes`
  10), **y es una compuerta de densidad en el eje TEMPORAL**: pregunta si hubo
  silencios. **Un muestreo regular cada 60 s no tiene silencios, la pasa, y después
  falla la cobertura de trazado** — por eso lo de C19 es **una compuerta nueva
  sobre una frontera vieja**, no una frontera nueva. El **modo apagado** también
  tiene precedente: el paso `llegada_fuera_ventana` se escribe con la nota
  «Informativo; no cambia el veredicto». Y la **métrica** ya está medida. **Falta
  solo** calcularla dentro del motor y anotarla como paso informativo, con su
  prueba. 🟢 **No toca ningún término de `servedRoute`** —es propiedad del conjunto
  de puntos, no de la ruta—, así que es seguro de medir por separado, como C12.
- **Y lo que el piso apagado deja aunque nunca se encienda:** que **cada hecho
  cargue la densidad con la que se le juzgó**. Es el Tramo 4 en pequeño — *el hecho
...
**13 de agosto de 2026 (C24 · la mitad barata, construida — y la pantalla tumbó
dos veces lo que yo había escrito).**

**Decidido por Asav: se arregla lo inmediato.** El expediente lee la política del
sello, como ya hacían el cierre, el diagnóstico y la tabla de ocurrencias.

- ✅ **`apps/web/src/lib/politica-del-sello.ts`** — decide qué política gobierna
  la lectura y **lo declara en `origen`** (`sello` / `contrato`), en vez de que
  quien lea lo suponga. Vive aparte de `service-detail-data.ts` por la misma
  razón que `identidad-dev.ts` vive aparte de `auth.ts`: **es la decisión que si
  se equivoca hace que una pantalla mienta sobre un hecho sellado**, y así se
  puede probar sola.
- 🟢 **Con prueba propia, y se comprobó que se pone roja:** devolviendo la
  política viva para un hecho sellado, **6 de 7 casos mueren**. Es la regla 8 —
  una defensa que ninguna prueba distingue de su ausencia no cuenta.
- 🟢 **Visto en el navegador, en los dos temas**, sobre uno de los 197 (Campus,
  17 de julio): el renglón **«Márgenes» pasó de `después 45` a `después 30`**,
  que es con lo que se juzgó, y la ventana de la política del sello de `06:35` a
  `06:20`.
- ✅ **Y de paso se cierra el ⚠ que dejó el #288: «lo que falta es que la pantalla
  diga cuál es cuál».** Ahora el renglón se llama **«Ventana de la política del
  sello»**, y cuando el contrato cambió después hay una línea que lo dice **sin
  dejar que eso mueva lo que se enseña**.

**Y lo que hay que registrar, porque es el segundo caso del día del mismo eje: la
pantalla tumbó dos veces lo que yo había escrito, y las dos veces era una causa
nombrada sin medir.**

- ⚠ **Primera:** al escribir el módulo, comparaba el snapshot **normalizado por
  el esquema** contra la política viva **en crudo**. Dos políticas idénticas
  salían «divergentes» por una llave que ninguna de las dos declaraba. **Es
  exactamente la trampa que el sensor de C24 se había comido del lado de SQL —el
  default hay que aplicarlo a los DOS lados—, repetida en TypeScript el mismo
  día por quien acababa de escribirla.** La atrapó su propia prueba.
- ⚠ **Segunda, y ésta la atrapó la captura de pantalla, no una prueba:** el
  renglón de la ventana del viaje decía «antes de tu cambio de política»
  —atribuía a una edición del contrato—; lo cambié a «la dimensionó la duración
  medida de la ruta» —atribuí a la derivación— **y la primera captura lo tumbó**:
  en ese servicio la ventana del viaje es **más ANGOSTA** que la de su política
  (05:50 contra 06:20), o sea que se congeló bajo una versión **anterior incluso
  a la del sello**. 🟢 **Hay tres épocas en juego —crear el viaje, sellar el
  hecho, hoy— y desde esa pantalla no se distinguen.** El renglón ahora **no
  nombra causa**: dice que no coincide y declara que no puede saber cuál de las
  dos fue. **Compilar no habría atrapado ninguna de las dos.**

⚠ **Lo que NO cierra este PR, y es la mayor parte de C24:**

- **La geocerca (519) no se arregla leyendo un snapshot.** Dibujar el polígono
  que el hecho nombra sería dibujar **uno que el motor nunca usó** — eso es C4, y
  su arreglo es congelar la forma dentro del hecho, no cambiar de qué fila se
  lee.
- **Los campos sin copia congelada siguen sin memoria:** ventana, etiquetas y
  placas de la unidad, nombres, estado del viaje y `evidence_points`. **Tramo 4.**
- **`calibrationViewEnd` en la pantalla del carrier sigue con la política viva.**
  No se tocó a propósito: es una vista de exploración para calibrar, no una
  afirmación sobre cómo se juzgó, y mezclarla aquí habría pedido su propia
  verificación visual.
...
**13 de agosto de 2026, cierre (C26, el piso en 60 s, y el paso 1 construido).**

- 🆕 **C26 — dos defensas que no coinciden en qué cuenta como cambio.** Escrita
  como causa y no como nota, por pedido de Asav. **Ninguna de las dos se ve mal
  por separado**, y ésa es toda la dificultad: el trigger dispara y la aplicación
  registra; **el hueco vive entre las dos**.
- ✅ **DECIDIDO por Asav: el piso de densidad va en 60 segundos.** Y con su razón
  escrita, que es lo que lo hace defendible: **60 es exactamente el intervalo con
  el que emitían los aparatos de Planta 47 los trece días que causaron todo
  esto**. El piso dice *«si el GPS reporta tan lento como en julio, no me atrevo
  a juzgar»*. **45 es demasiado agresivo —alcanzaría a 203 servicios— y 90 no
  toca a nadie: la póliza no existiría.**
- ✅ **Paso 1 construido: la densidad se calcula, se anota y se congela.**
  `medirDensidad` usa la **misma definición que `medir-cadencia`** —mediana del
  hueco entre puntos consecutivos **del mismo aparato**— y no puntos÷duración: un
  cociente se mueve por el denominador y **ya engañó una vez**.
- 🟢 **Y no gobierna, con prueba que lo fija:** la misma geometría con evidencia
  cada 10 s y cada 120 s **da el mismo veredicto**. Si esa prueba se pone roja
  sin que nadie encienda el paso 4, **la densidad empezó a decidir sin que se
  decidiera**.
- 🟢 **El paso del ledger declara `gobierna: false`.** Un número junto a un
  veredicto invita a leerse como causa, y hasta el paso 4 no lo es.
- ⚠ **Columna propia (`0023`) y NO dentro de `candidatas_snapshot`**, aunque
  saldría más barato: aquél dice «candidatas» y la densidad es propiedad de la
  EVIDENCIA. **Un campo cuyo nombre no describe su contenido es C15 y C20 otra

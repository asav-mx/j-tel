...
- **Tino** (nombre provisional; id interno `paradito`) es la parada oficial, un poste. Toma el color de su ruta.
...
- **Naranja:** solo Ontoy.
- **Color de ruta:** lo elige la concesión (`color_hex`). No se predetermina ni se limita. El sistema lo ajusta lo mínimo para que quede lejos del naranja, sea visible y se lea (el texto va en blanco o carbón según el contraste; ver `contraste-de-ruta.ts` en el repo). Siempre va con su placa.
...
- No se usa ninguna fuente de íconos genérica. Si hace falta un ícono de interfaz, marca el hueco y pregunta.

## Pendientes
- El nombre definitivo de Tino.

...
import { JTTEL_TZ, localDateIso, instanteZonificado } from "./tiempo.js";
 ...
 *
 * Usa `node:crypto` para el identificador opaco, y este índice lo cargan
 * componentes que corren en el navegador. Reexportarlo tumbó el build de la app
 * web con «UnhandledSchemeError: node:crypto» — un error de empaquetado, no de
 * tipos, así que `tsc` lo dejó pasar y solo apareció en `pnpm build`.
 *
 * Se importa por subruta, `@jtel/domain/publico`, que además dice sola que eso
 * es código de servidor.
 */
